...
def plot_rrt_tree(grid_map: GridMap, tree, file_name: str,
                  path: list = None, title: str = "") -> None:
    """
    成長させた探索木（ノード＋枝）を地図に重ねて描いて保存する
    （path を渡すと，ゴールへ届いた復元経路を太線で強調して重ねる）

    パラメータ
        grid_map: 占有格子地図
        tree: 探索木
        file_name: 保存ファイル名
        path: 復元経路（連続座標 (x, y) のリスト）。None なら木だけを描く
        title: 図のタイトル
    """
    fig, ax = plt.subplots(figsize=(6, 6))
    _draw_base(ax, grid_map)
...
def animate_rrt(grid_map: GridMap, tree, path: list,
                file_name: str, step: int = 3, title: str = "") -> None:
    """
    木が枝を数本ずつ伸ばして広がり，最後にゴールへ届いた経路を引く様子を gif で保存する
    ...
    # 最後のコマをくり返して，引けた経路を約1.5秒だけ見せてから先頭へ戻す（gif はループ再生のため）
    frames = [("grow", k) for k in grow_frames] + [("path", n)] * _hold_frames(8)
    ...
    anim = FuncAnimation(fig, update, frames=frames, interval=100, blit=False)
    anim.save(file_name, writer="pillow", fps=8)
    plt.close(fig)
