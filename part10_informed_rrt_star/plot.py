...
# --- ここから第10章（Informed RRT*）用：楕円体サンプリング領域の描画 ---

def _ellipse_points(ellipse: tuple, n_points: int = 100) -> np.ndarray:
    """
    楕円（スタート・ゴールを焦点とするサンプリング領域）の輪郭の点列を作る（内部用）

    単位円の点を「長径 c_best/2・短径 √(c_best²−c_min²)/2 の対角行列 L」で引き伸ばし，
    回転行列 C で向きをそろえ，中心 center へ運ぶ（informed サンプリングと同じ変換）。

    パラメータ
        ellipse: (center, C, c_best, c_min) … 楕円の中心・回転行列・長径のもと・焦点間距離
        n_points: 輪郭を何点で描くか（多いほど滑らか）

    戻り値
        points: 楕円の輪郭点列（shape=(n_points, 2) の (x, y) 配列）
    """
    center, C, c_best, c_min = ellipse
    r1 = c_best / 2.0                                      # 長径（横方向）
    ri = np.sqrt(max(c_best ** 2 - c_min ** 2, 0.0)) / 2.0  # 短径（縦方向）
    L  = np.diag([r1, ri])

    theta  = np.linspace(0.0, 2.0 * np.pi, n_points)
    circle = np.stack([np.cos(theta), np.sin(theta)], axis=0)   # 単位円の輪郭（2×n）
    points = (C @ L @ circle).T + center                        # 楕円へ変換して中心へ移動
    return points


def _draw_ellipse(ax: Axes, ellipse: tuple, color: str = "tab:green",
                  label: str = "sampling ellipse") -> None:
    """
    サンプリング領域の楕円を地図の上に重ねて描く（内部用）

    パラメータ
        ax: 描画先の Axes
        ellipse: (center, C, c_best, c_min)
        color: 楕円の輪郭の色
        label: 凡例ラベル
    """
...
def plot_informed_path(grid_map: GridMap, tree, path: list, ellipse: tuple,
                       informed_samples: list, file_name: str, title: str = "") -> None:
    """
    探索木（薄め）・楕円体内サンプル・最終的な楕円・最適経路を重ねて描いて保存する

    楕円の内側にサンプル（小さな点）が集中していることと，その中で経路が最短へ最適化された
    ことを1枚で見せる（Informed RRT* が「無駄なサンプルを捨てている」ことが分かる図）。

    パラメータ
        grid_map: 占有格子地図
        tree: 探索木
        path: 経路（連続座標 (x, y) のリスト）
        ellipse: 最終的な楕円 (center, C, c_best, c_min)
        informed_samples: 楕円体内からサンプルした点のリスト
        file_name: 保存ファイル名
        title: 図のタイトル
    """
    fig, ax = plt.subplots(figsize=(6, 6))
    ...
    # 楕円体内からのサンプル（薄い緑の小点）が楕円の中に集まっている様子
    if informed_samples:
        s = np.array(informed_samples)
        ax.plot(s[:, 0], s[:, 1], ".", color="tab:green",
                markersize=2, alpha=0.35, zorder=2, label="informed samples")

    # 最終的なサンプリング楕円
    if ellipse is not None:
        _draw_ellipse(ax, ellipse)
...
def animate_informed(grid_map: GridMap, tree, snapshots: list,
                     file_name: str, title: str = "") -> None:
    """
    最良経路が短くなるたびに，サンプリング楕円が細く縮んでいく様子を gif で保存する

    パラメータ
        grid_map: 占有格子地図
        tree: 成長させた Informed RRT* 探索木（背景に薄く描く）
        snapshots: (試行回数, コスト, 経路, 楕円) のリスト（最良経路が更新されるたびの記録）
        file_name: 保存ファイル名
        title: 図のタイトル
    """
    fig, ax = plt.subplots(figsize=(6, 6))
    _draw_base(ax, grid_map)
    # 背景に最終的な木を薄く描く（楕円と経路の変化が際立つように）
    _draw_tree(ax, tree, edge_alpha=0.15)

    path_plot,    = ax.plot([], [], "-", color="tab:orange", linewidth=2.5, zorder=5)
    ellipse_plot, = ax.plot([], [], "-", color="tab:green", linewidth=1.8, zorder=4)
    text = ax.text(0.02, 0.02, "", transform=ax.transAxes, fontsize=10,
                   color="tab:orange", verticalalignment="bottom")

    # 各スナップショットを順に見せ，最後の最良経路（最も細い楕円）を少し長く映す
    # このアニメは fps=3（1コマ 約0.33秒）なので，6コマで約2秒。すでに _GOAL_HOLD_SEC を満たす
    frames = list(range(len(snapshots))) + [len(snapshots) - 1] * 6

    def update(k: int) -> tuple:
        iteration, cost, path, ellipse = snapshots[k]
        p = np.array(path)
        path_plot.set_data(p[:, 0], p[:, 1])
        # その時点の c_best に対応する楕円（コストが下がるほど細くなる）
        points = _ellipse_points(ellipse)
        ellipse_plot.set_data(points[:, 0], points[:, 1])
        text.set_text(f"iter={iteration}  cost={cost:.2f}")
        return path_plot, ellipse_plot, text
...
def plot_cost_compare(histories: list, file_name: str, title: str = "") -> None:
    """
    複数手法の「反復回数 vs 最良経路コスト」の収束カーブを重ねて描いて保存する

    RRT* と Informed RRT* を同じ地図・同じシードで比べ，初期解の後にどちらが速く
    コストを下げるか（＝楕円体サンプリングの効き）を見せる。

    パラメータ
        histories: [(手法名, 色, cost_history), ...] のリスト
                   cost_history は (試行回数, 最良コスト) のリスト
        file_name: 保存ファイル名
        title: 図のタイトル
    """
    fig, ax = plt.subplots(figsize=(6, 4))

    for name, color, cost_history in histories:
        if not cost_history:
            continue
        iters = [c[0] for c in cost_history]
        costs = [c[1] for c in cost_history]
        # コストは「下がったら据え置き」の階段状なので step で描く
        ax.step(iters, costs, where="post", color=color, linewidth=1.8,
                label=f"{name} (final={costs[-1]:.2f})")
...
def plot_ellipse_concept(grid_map: GridMap, ellipses: list, file_name: str,
                         title: str = "") -> None:
    """
    複数の c_best に対応する楕円を入れ子で描き，「c_best が縮むほど楕円が細くなる」概念を見せる（理論編用）

    パラメータ
        grid_map: 占有格子地図（スタート・ゴールを焦点として描く）
        ellipses: [(ラベル, 色, ellipse), ...] のリスト（ellipse=(center, C, c_best, c_min)）
        file_name: 保存ファイル名
        title: 図のタイトル
    """
    fig, ax = plt.subplots(figsize=(6, 6))
    ...
    for label, color, ellipse in ellipses:
        points = _ellipse_points(ellipse)
        ax.plot(points[:, 0], points[:, 1], "-", color=color,
                linewidth=1.8, zorder=4, label=label)
